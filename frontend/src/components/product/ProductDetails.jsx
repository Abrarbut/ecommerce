                ...
                <div className="flex flex-col md:flex-row">
                    ...
                    <div className="hidden md:flex flex-col space-y-4 mr-6">
                        {selectedProduct.images.map((image, index) => (
                            <div key={index} className="w-24 h-24 rounded-md overflow-hidden cursor-pointer">
                                <img
                                    key={index}
                                    src={image.url}
                    ...
                    <div className="md:w-1/2 ">
                        <div className="mb-4">
                            <img src={mainImage} alt="Main Product"
                                className="h-full h-auto object-cover rounded-lg" />
                        </div>
                    </div>
                    {/* mobile thumbnail */}
                    <div className="md:hidden flex overscroll-x-scroll space--4 mb-4">
                        {selectedProduct.images.map((image, index) => (
                            <div key={index} className="w-24 h-24 rounded-md overflow-hidden cursor-pointer">
                                <img
                                    key={index}
                    ...
                    <div className="text-2xl md:ml-10">
                        ...
                        <p className="text-al text-gray-500 mb-2">
                            ...
                            <div className="flex-gap mt-2">
                                {selectedProduct.sizes.map((size) => (
                                    <button key={size}
                                        ...
                                        className={`px-4 py-2 rounded border ${selectedSize === size ? "bg-black text-white" : "bg-white text-gray-700"}`}>{size}</button>
                                ))}
                            </div>
                        </div>
                        <div className="mb-6">
                        ...
                        className={`bg-black text-white py-2 px-6 rouded w-full mb-4 
